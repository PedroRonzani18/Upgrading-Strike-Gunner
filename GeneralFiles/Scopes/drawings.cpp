...
void initializeTextures()
{
    ...
    std::vector<std::vector<std::string>> valores = Parser::parseTextureData("Assets/Scripts/Textures/data.txt");

    for(auto vec: valores)
    {
        if(vec[4] == "-1")
        {
            textureModule(templateSquare, std::stof(vec[0]), std::stof(vec[1]), loadTexture(stringToArray(vec[2])));
            std::cout << vec[0] << " " << vec[1] << " " << vec[2] << std::endl;
        }
        else
        {
            textureModule(templateSquare, std::stof(vec[0]), std::stof(vec[1]), loadTexture(stringToArray(vec[2])), std::stof(vec[3]), std::stof(vec[4]), std::stof(vec[5]), std::stof(vec[6]));
            std::cout << vec[0] << " " << vec[1] << " " << vec[2] << " " << vec[3] << " " << vec[4] << " " << vec[5] << std::endl;
        }
    }

    // Cria um quadrado com dimensoes x y, carrega a textura em questão e armazena os IDs gerados no vector "textures"
    // std::cout << valores[0][2] << "AA\n";
    // std::cout << "Assets/Images/ajudaNew.pngAA\n";
    //         textureModule(templateSquare, std::stod(valores[0][0]), std::stod(valores[0][1]), loadTexture(stringToArray(valores[0][2])));
