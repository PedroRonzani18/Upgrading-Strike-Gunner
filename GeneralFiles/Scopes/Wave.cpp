...
std::vector<Enemy> waveTemplateGeneral(const char* path)
{
    ...
    {       
        if(!aP.randMinus.empty())   for(int j = 0; j < aP.numberOfEnemies[i]; j++)
        ...
        for(int j = 0; j < aP.numberOfEnemies[i]; j++)
        {
            ...
            if(!aP.randMinus.empty())
                e1.setMidPoint(aP.midConstX[i] * rands[j] + j * aP.multiplyerX[i], aP.midConstY[i] + j * aP.multiplyerY[i]);
            
            else
               e1.setMidPoint(aP.midConstX[i]             + j * aP.multiplyerX[i], aP.midConstY[i] + j * aP.multiplyerY[i]); 
